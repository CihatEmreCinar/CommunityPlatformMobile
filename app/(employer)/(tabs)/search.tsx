    ...
    <ScreenContainer edges={['top', 'bottom']} header={<Text style={styles.title}>Mekan Bul</Text>} scroll={false}>
      <KeyboardAwareScreen contentContainerStyle={styles.content}>
        <CollapsibleFilterPanel title="Filtreler" summary={filterSummary} defaultOpen>
          ...
          <View style={styles.filterGroup}>
            ...
            <View style={styles.rangeRow}>
              <TextInput style={[styles.input, styles.rangeInput]} value={minCapacity} onChangeText={setMinCapacity} keyboardType="number-pad" placeholder="Min" placeholderTextColor={Colors.outline} />
              <Text style={styles.rangeSeparator}>–</Text>
              <TextInput style={[styles.input, styles.rangeInput]} value={maxCapacity} onChangeText={setMaxCapacity} keyboardType="number-pad" placeholder="Max" placeholderTextColor={Colors.outline} />
            </View>
          </View>
          ...
          <View style={styles.filterGroup}>
            ...
            <View style={styles.rangeRow}>
              <TextInput style={[styles.input, styles.rangeInput]} value={minPrice} onChangeText={setMinPrice} keyboardType="number-pad" placeholder="Min" placeholderTextColor={Colors.outline} />
              <Text style={styles.rangeSeparator}>–</Text>
              <TextInput style={[styles.input, styles.rangeInput]} value={maxPrice} onChangeText={setMaxPrice} keyboardType="number-pad" placeholder="Max" placeholderTextColor={Colors.outline} />
            </View>
          </View>
        ...
        {searched && !loading && listings.length > 0 ? (
          <Text style={styles.resultInfo}>{searchResult.total} sonuç bulundu · Sayfa {searchResult.page}</Text>
        ) : null}
        ...
        <View style={styles.list}>
          {listings.map((listing) => (
            <TouchableOpacity key={listing.id} style={styles.card} onPress={() => router.push(`/(employer)/space/${listing.id}` as any)} activeOpacity={0.85}>
              <View style={styles.imagePlaceholder}><Icon name="place" size={22} color={Pastel.coral.text} /></View>
...
const styles = StyleSheet.create({
  ...
  card: { flexDirection: 'row', alignItems: 'center', gap: Spacing.md, backgroundColor: Pastel.coral.tint, borderRadius: Radius.xxl, padding: Spacing.md },
  imagePlaceholder: { width: 56, height: 56, borderRadius: Radius.full, backgroundColor: Pastel.coral.tintStrong, alignItems: 'center', justifyContent: 'center' },
  cardBody: { flex: 1, gap: 4 },
  cardTitle: { ...Typography.labelMd, color: Colors.onSurface },
  cardMeta: { ...Typography.bodyMd, color: Colors.onSurfaceVariant },
  cardText: { ...Typography.bodyMd, color: Pastel.coral.text, fontWeight: '700' },
  emptyText: { ...Typography.bodyMd, color: Colors.onSurfaceVariant, textAlign: 'center', marginTop: Spacing.md },
  loadMoreButton: { alignSelf: 'center', backgroundColor: Pastel.coral.tint, borderRadius: Radius.full, paddingHorizontal: Spacing.md, paddingVertical: Spacing.sm, marginTop: Spacing.sm },
  loadMoreText: { ...Typography.labelMd, color: Pastel.coral.text },
});
