...
export interface WorkshopFormProps {
  mode: 'create' | 'edit';
  /** Edit modunda formu doldurmak için mevcut atölye verisi. */
  initialWorkshop?: Workshop;
  /** Create → workshopService.create, Edit → workshopService.update(id, ...) çağırır. */
...
export function WorkshopForm({ mode, initialWorkshop, onSubmit }: WorkshopFormProps) {
  ...
  const [locationType, setLocationType] = useState<'online' | 'in-person'>(
    initialWorkshop?.locationType ?? 'in-person'
  );
  ...
  const [location, setLocation] = useState<LocationSelection>(
    initialWorkshop
      ? {
          cityId: initialWorkshop.cityId,
          cityName: initialWorkshop.city,
          districtId: initialWorkshop.districtId,
          districtName: initialWorkshop.district,
        }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        Alert.alert('Başarılı', 'Atölye güncellendi.', [
          { text: 'Tamam', onPress: () => router.back() },
        ]);
      }
    } catch (error: any) {
      const message =
        ...
        (mode === 'create' ? 'Atölye oluşturulamadı.' : 'Atölye güncellenemedi.');
